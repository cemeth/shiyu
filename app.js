...
let spellPureListen = false;           // 拼写纯听力模式(隐藏中文提示)
let stressVisible = true;              // 卡片正面是否显示重音符号
let toastTimer = null;
...
function launchConfetti() {
  const colors = ['#f59e0b', '#ef4444', '#15803d', '#1a56db', '#8b5cf6'];
  for (let i = 0; i < 50; i++) {
...
function loadState() {
  try {
    ...
    spellPureListen = localStorage.getItem(pkey('spell_pure_listen')) === '1';
    stressVisible = localStorage.getItem(pkey('stress_visible')) !== '0';
  } catch (e) {
    ...
    spellPureListen = false; stressVisible = true;
  }
}
...
function renderCard() {
  ...
  if (curWord) {
    const ruDisplay = stressVisible ? curWord.ru : norm(curWord.ru);
    $('#card-ru').textContent = ruDisplay;
...
function setSpellMode(m) {
  spellMode = m;
  localStorage.setItem(pkey('spell_mode'), m);
...
function showSpellQuestion() {
  ...
  if (spellMode === 'listen') {
    if (spellPureListen) {
      ...
      $('#spell-zh').classList.add('pure-listen');
    } else {
      $('#spell-zh').textContent = '🎧 听发音,拼出你听到的单词';
      $('#spell-zh').classList.remove('pure-listen');
    }
...
function renderDueView() {
  ...
  if (reviewWord) {
    ...
    $('#review-ru').textContent = ruDisplay;
...
// ========== SPA 路由：hash 驱动面板切换 ==========
let _spaInitPending = false;
function switchPanel(target) {
  const page = target || getSPAPage();
  history.replaceState(null, '', '#' + page);
  // 如果语言还没加载完,等初始化完成后再渲染
  if (!lang) {
    _spaInitPending = true;
    _spaPendingPage = page;
    return;
  }
  _doSwitchPanel(page);
}
function _doSwitchPanel(page) {
  document.querySelectorAll('.spa-panel').forEach(p => p.classList.remove('active'));
  const panel = document.querySelector('.spa-panel#panel-' + page);
  ...
  window.scrollTo(0, 0);
  _spaInitPending = false;
  _spaPendingPage = null;
}

document.addEventListener('hashchange', () => {
  const page = getSPAPage();
  if (lang) _doSwitchPanel(page);
  else { _spaInitPending = true; _spaPendingPage = page; }
});
...
(function initSPARoute() {
  const page = getSPAPage();
  document.body.dataset.page = page;
  // 保存当前 hash 供 initAll 调用后使用
  window.__SPA_PENDING_PAGE = page;
...
function initAll(newLang, quiet) {
  ...
  // SPA:激活当前面板,恢复切换前等待的页面
  if (_spaInitPending && _spaPendingPage) {
    _doSwitchPanel(_spaPendingPage);
  } else {
    _doSwitchPanel(getSPAPage());
  }
...
// 拼写纯听力模式开关
on('#spell-pure-listen', 'click', toggleSpellPureListen);
// 错词专项练习
on('#wrongdrill-begin', 'click', () => startWrongDrill(8));
on('#wrongdrill-input', 'keydown', (e) => {
  if (e.key === 'Enter') {
    e.preventDefault();
    if (!$('#wrongdrill-next').hidden) wrongDrillNext(); else submitWrongDrill();
  }
});
on('#wrongdrill-submit', 'click', submitWrongDrill);
on('#wrongdrill-next', 'click', wrongDrillNext);
on('#wrongdrill-again', 'click', () => startWrongDrill(8));
on('#wrongdrill-speak', 'click', () => { if (wrongDrill) speak(wrongDrill.words[wrongDrill.idx].ru); });
...
on('#stress-toggle', 'click', toggleStress);
on('#quiz-next', 'click', () => {
  quiz.idx++;
...
function toggleStress() {
  ...
  localStorage.setItem(pkey('stress_visible'), stressVisible ? '1' : '0');
  const btn = $('#stress-toggle');
  if (btn) btn.classList.toggle('active', stressVisible);
  const label = btn ? btn.querySelector('.stress-label') : null;
  if (label) label.textContent = stressVisible ? '重音:开' : '重音:关';
  if (curWord) renderCard();
  if (reviewWord) renderDueView();
}

// ========== 拼写纯听力模式开关 ==========
function toggleSpellPureListen() {
  spellPureListen = !spellPureListen;
  localStorage.setItem(pkey('spell_pure_listen'), spellPureListen ? '1' : '0');
  const btn = $('#spell-pure-listen');
  if (btn) btn.classList.toggle('active', spellPureListen);
  if (spell) showSpellQuestion(); // 实时刷新当前题
}

// ========== 错词专项练习 ==========
let wrongDrill = null; // {words, idx, score, wrong}
function startWrongDrill(count) {
  const pool = wrongBookList();
  if (pool.length < 3) { toast('错词太少,先多练几轮吧'); return; }
  const n = Math.min(count || 8, pool.length);
  shuffle(pool);
  wrongDrill = { words: pool.slice(0, n), idx: 0, score: 0, wrong: [] };
  $('#wrongdrill-start').hidden = true;
  $('#wrongdrill-run').hidden = false;
  $('#wrongdrill-result').hidden = true;
  showWrongDrillQuestion();
}
function showWrongDrillQuestion() {
  const w = wrongDrill.words[wrongDrill.idx];
  $('#wrongdrill-qnum').textContent = '第 ' + (wrongDrill.idx + 1) + ' 题 / 共 ' + wrongDrill.words.length + ' 题';
  $('#wrongdrill-score').textContent = '得分:' + wrongDrill.score;
  $('#wrongdrill-ru').textContent = w.ru;
  $('#wrongdrill-hint').innerHTML = '<span class="book-zh">' + esc(w.zh) + '</span>' + (w.note ? ' <span class="sub-tip">' + esc(w.note) + '</span>' : '');
  $('#wrongdrill-input').value = '';
  $('#wrongdrill-input').disabled = false;
  $('#wrongdrill-submit').disabled = false;
  $('#wrongdrill-feedback').innerHTML = '';
  $('#wrongdrill-feedback').className = 'spell-feedback';
  $('#wrongdrill-next').hidden = true;
  $('#wrongdrill-input').focus();
  speak(w.ru);
}
function submitWrongDrill() {
  if (!wrongDrill) return;
  const w = wrongDrill.words[wrongDrill.idx];
  const ans = norm($('#wrongdrill-input').value);
  if (!ans) { toast('先输入答案再提交'); return; }
  const fb = $('#wrongdrill-feedback');
  if (ans === norm(w.ru)) {
    wrongDrill.score++;
    fb.className = 'spell-feedback ok';
    fb.innerHTML = '✓ 正确!';
  } else {
    wrongDrill.wrong.push(w);
    fb.className = 'spell-feedback no';
    fb.innerHTML = '✗ 正确答案:<span class="spell-correct-word">' + esc(w.ru) + '</span>(' + esc(w.zh) + ')';
  }
  $('#wrongdrill-input').disabled = true;
  $('#wrongdrill-submit').disabled = true;
  $('#wrongdrill-next').hidden = false;
}
function wrongDrillNext() {
  wrongDrill.idx++;
  if (wrongDrill.idx >= wrongDrill.words.length) finishWrongDrill();
  else showWrongDrillQuestion();
}
function finishWrongDrill() {
  $('#wrongdrill-run').hidden = true;
  ...
  quizLog[qd].count += 1;
  saveQuizLog();
  toast('错词练习完成!对了 ' + score + '/' + total);
}
function wrongBookList() {
  return allWords().filter((w) => (reviewStateOf(w)?.wrong || 0) > 0 || quizWrong.includes(wkey(w)));
}
...
// ========== 键盘快捷键 ==========
// 背单词页: Space翻卡, ←/→翻页, 1=不认识 2=模糊 3=认识
// 复习页: 同上, + Enter提交拼写答案
document.addEventListener('keydown', (e) => {
  ...
  if (currentPage === 'home' && curWord) {
    ...
    else if (key === '1') { e.preventDefault(); markNew(); }
    else if (key === '2') { e.preventDefault(); markFamiliar(); }
  ...
  // 复习页快捷键
    ...
    else if (key === '1') { e.preventDefault(); reviewStillNew(); }
